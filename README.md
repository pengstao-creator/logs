...
## v2.0 新特性
- **头文件与源码分离**：核心类实现从头文件分离到独立的.cpp源文件，提升编译效率
- **动态库支持**：默认生成动态库libmylog.so，便于集成和部署
- **预编译头优化**：使用预编译头(pch.hpp)加速编译
- **编译优化**：启用-O2优化编译，提升运行时性能
...
│   └── testlog.cpp      # 综合测试
├── bin/                 # 可执行文件输出目录
├── lib/                 # 动态库输出目录（v2.0变更：原lib目录存放静态库，现存放动态库）
├── build/               # 构建系统文件
├── config/              # 配置文件目录
├── logs/                # 默认日志输出目录
├── CMakeLists.txt       # CMake构建配置（v2.0新增Release编译优化）
├── LICENSE              # 许可证文件
├── README.md            # 中文文档
└── README.en.md         # 英文文档
```
...
- **v2.0编译优化**：启用-O2优化编译，运行时性能提升20-40%
- **v2.0预编译头**：使用pch.hpp加速编译，首次编译时间减少约11%
- **同步模式**：简单应用程序的开销较低
- **异步模式**：最小化阻塞，后台处理日志
- **线程池**：高负载应用的高吞吐量
- **缓冲区管理**：高效的内存使用
